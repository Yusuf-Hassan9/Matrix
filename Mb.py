    ...
    while True:
        if Move1 == 'a':
            ...
            Move = get_valid_input("[A] Try to hurdle over the spikes?\n[B] "
                 "Look for a way to cover the spikes?\n", ['a', 'b']).lower()

            while True:
                if Move == 'a':
                    # Option to hurdle over the spikes
                    sprint("You brace yourself and prepare to leap over the"
                          " spikes.\n")
                    sprint("With a burst of energy, you jump, trying to clear"
                          " the sharp points...\n")
                    hurdle_success = random.choice([True, False]) # Random chance for success
        
                    if hurdle_success:
                        sprint("You manage to Successfully Hurdle over the "
                               "spikes.\n")
                        print(f"'{player.name} you Really avoided those spikes"
                              " like a Ninja'")
                        sprint("You can see a entrance into the Building.\n")
                        input("Press ENTER to go to the entrance \n")
                        sprint("You are now at the entrance, And to your "
                          "suprise there are no Matrix Guards to be seen.\n")
                        ...
                        sprint("You Found some Cloth in the Container and used"
                              " it to patchup your wound.\n")
                    ...
                    sprint("You climb over using the Branch as cover, avoiding"
                     " the sharp points, and you make it to the other side "
                     "safely!\n")
                    print(f"{ai}\n")
            ...
            sprint("Keeping low to the ground, you move silently in the shadows"
                   ".\n")
            sprint("As you get closer, you spot a Matrix Guard near the "
                   "entrance.\n")
            ...
            Move = input("[A] Distract the Matrix Guard?\n[B] Wait for the"
             " Matrix Guard to turn away, Then take him out quickly?\n").lower()

            while True:
                if Move == 'a':
                    sprint("You Find some Pebbles on the ground, You then use "
                        "the Pebbles to distact the guard by throwing them.\n")
                    sprint("You swiftly Begin Moving to the Entrance.\n")
                    Entrance_success = random.choice([True, False]) # Random chance for success

                    if Entrance_success:
                        sprint("Luckily, You make it to the Entrance without "
                               "getting caught.\n")
                        input("Press ENTER to go inside the Matrix Base")
                        ...
                        Move = get_valid_input("[A] Turn and Knock Out the "
                                               "Henchman"
                         " using your Fists?\n[B] Turn and Reach for your"
                        " Weapon to take him out?\n", ['a', 'b'])

                        while True:
                            if Move == 'a':
                               sprint(f"{player.name} you turn around smoothly and"
                                " give the Matrix Guard a punch right on the "
                                "nose.\n")
                               sprint("The Matrix Guard was Instantly Knocked"
                                     " out Clean!\n")
                               print(f"{ai}\n")
                               sprint("What a Hit, I am Certainly Impressed. "
                                      "\n")
                               sprint(f"{player.name} you must quickly tie up "
                                      "this body, Then head into the Base. \n")
                               input("Press ENTER to tie up the Matrix Guards "
                                     "Body"
                                    "\n")
                               sprint("The Matrix Guards body has been dealt "
                                      "with.\n")
                               input("Press ENTER to go inside the Matrix Base "
                                     "\n")
                               break
                                ...
                                sprint("You Turned Around Smoothly, However as "
                    ...
                    sprint("You wait for the Matrix Guard to turn around and "
                           "Knock him out clean.\n")
        ...
        if Move == 'a':
            ...
            if Move == 'a':
                ...
                sprint("'If you want to go to the files room, Go Downstairs "
                       "'\n")
                sprint("'then take a right and go down to the end of the "
                       "corridor!'"
                       "\n")
                ...
                sprint("'You must hurry and get there, Before the Matrix guard"
                      " wakes up and the whole place is alerted!'\n")
                ...
                sprint("'I've never actually guarded the files rooom before "
                       " ...'\n")
                sprint("'All i know is that its all the way downstairs and on "
                       "the"
                        " right'\n")
            ...
            sprint(f"{player.name}, you hear faint footsteps approaching from "
                   "around"
                   " the corner.\n")
            sprint("You peek your head and see that there is a Matrix guard.\n")
            print(f"{player.name} you can use this as a opportunity to find "
                  "out where the Files room is. \n")
            ...
            while True:
                if Move == 'a':
                    sprint("You sneak up to the guard from behind, moving as"
                           " silently as possible.\n")
                    sprint("You press your weapon to the back of his head and "
                    ...
                    sprint("'Good Work there.'\n")
                    sprint("'You now know where to find the files room'\n")
                    sprint("'Go to the files room, Before the Matrix guard "
                           "wakes up and the whole place is alerted!'\n")
                    ...
                    sprint("The Matrix guard is now talking into his earpiece."
                           "\n")
                    print(f"{mg}\n")
                    sprint("'Boss said im switching with you in 20 mins to "
                           "guard the...'\n")
                    sprint("'Thats Alright, I can tell you where it is ...'\n")
                    sprint("'The files room is here, Downstairs and on the"
                    ...
                    sprint("Invalid Choice, Choose a option!\n")
                    Move = input("[A] Sneak up behind and point your weapon"
                    " to his"
                    " head?\n[B] Stay Hidden for abit and watch the guard?"
                     "\n").lower()
...
def DataStick():  # Finding the Data Stick Mini Scene #
    ...
    if Move == 'a':
        ...
        if Move == 'a':
            sprint("You look in the lockers, But the stick is still not "
                   "visible.\n")
    ...
    # Searching the Computer
        ...
        if Move == 'a':
             ...
             sprint("All you can really do now is look around the room and"
                   " hope for the best. \n")
             sprint("Time is really Running out ... \n")
             input("Press ENTER to look around the room \n")
             sprint("you are looking around desperately now, When ... \n")
            ...
            sprint("Invalid Output!\n")
            Move = input("[A] Search The Computer Desk for the Stick?\n[B] "
                         "Look around the room Hoping to Find it?\n").lower()

    else:
        sprint("Invalid Output!\n")
        Move = input("[A] Search The Computer Desk for the Stick?\n[B] Search "
                     "the Lockers for the Stick?\n").lower()
